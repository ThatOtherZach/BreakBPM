...
function toHistoryEntry(
  ...
  const gsPlayers = Array.isArray(gs?.["players"])
    ? (gs!["players"] as Array<{ team?: string }>)
    : [];
  const rawGroup =
    subject.slot != null && subject.slot >= 0 && subject.slot < gsPlayers.length
      ? gsPlayers[subject.slot]?.team
      : undefined;
