...
function buildPassesBody(): string {
  const plans = [
    ...
    { name: "30 Day Pass", price: "$4.99", suffix: "", desc: "30 days of full access. Pay by card on our store — we email your redeem code within 24 hours.", note: "Card-only alternative — buying ~30 days of access with crypto (~$5.69) is the better deal." },
  ];
...
function poolStatsAppJsonLd(): string {
  const softwareApp = {
    ...
    offers: [
      ...
      { "@type": "Offer", name: "30 Day Pass", price: "4.99", priceCurrency: "USD", description: "30 days of full access. Paid by card via our off-platform store; redeem code emailed within 24 hours." },
    ],
  };
  const faqPage = {
    "@context": "https://schema.org",
