...
- `/stats` shows accuracy, pace, defense (% of shots that were winning safeties — safeties after which the opponent's next shot pocketed nothing; the volume-aware pair to accuracy), and ball/pattern breakdowns (retro CRT styling) with tier-gated windows. Recent Chaos winners get a rainbow AVG-BPM flourish.
