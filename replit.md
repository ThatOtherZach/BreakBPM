...
- Optional env (restart the api-server workflow after changing any):
  ...
  - `BREAKBPM_STORE_URL` — off-platform card store (Squarespace) URL for the **30 Day Pass** ($4.99 / 30d — same price as 30 days via crypto); read per-request via `GET /config`. Empty when unset → the PassesScreen card-store callout is hidden. Buyer pays by card on the store; owner manually mints a "30 Day Pass" admin redeem code and emails it (≤24h). Crypto at 30 days grants instantly.
  ...
  - `BREAKBPM_INVITE_TRIAL_HOURS` — length (in hours) of the free trial pass granted to a NEW user who signs up via an invite link (`/invite/{code}`); whole number, min 1 (default `24`; blank/invalid → warn + default). Shipped to the client as `InviteCodeResult.trialLabel` (e.g. "24-hour") so the AccountScreen invite copy never drifts.
...
- **Home ("/") and per-hall prerender**: the root `index.html` and every active Verified Hall's `/leaderboard/hall/<slug>` page also get real static HTML + JSON-LD at build time (same `closeBundle` pipeline as the other `PUBLIC_ROUTES`). Home body copy lives in `landingContent.ts` (`HOME_*`); hall pages are generated from a direct build-time Postgres query (raw `pg`, not `@workspace/db`, which throws without `DATABASE_URL`) since venues are DB rows, not static copy — a missing/failed DB read just skips hall-page generation, never fails the build, so newly-minted halls only get a static page on the next deploy. Home is written LAST from the pristine root `index.html` (after all subroute/hall pages read it) and carries a guard script that removes the injected body on any other path, because the static host's catch-all rewrite (`/* → /index.html`) would otherwise flash homepage copy before React mounts on unmatched client routes.
...
- **List your hall (free)**: pool halls can claim a free Verified-Hall listing via the `/for-venues` pitch page — their own Local Leaderboard, map discovery, and a website backlink in exchange for one social media post tagging #BreakBPM with a link to their hall page.
...
- **Passes (crypto + redeem codes)**: free to play; sign in (free) to save stats. With crypto you buy **Purchase Days of Access** — any 1–365 days priced on marginal per-day brackets that get cheaper the more days you add (a single day is $1.99; **30 days is $4.99** — same as the card 30 Day Pass; the whole bracket set is env-tunable via `BREAKBPM_DAY_PASS_*`, shipped to the client via `/passes/plans`) — plus a **Lifetime** pass ($24.99) and **Lucky Break** ($4.99); none auto-renew. The original fixed Day/Month/Year/Lifetime one-time pass *kinds* still exist (the flexible pass issues a `day` kind with a custom duration, and redeem codes can still grant any kind), but Day/Month/Year are no longer sold directly via crypto — only the flexible day pass, Lifetime, and Lucky Break are crypto-buyable. A fifth **30 Day Pass** ($4.99 / 30d, internal kind `twoweek` — legacy name predating the current 30-day terms) is also sold *off-platform by card* via the owner's Squarespace store (`BREAKBPM_STORE_URL`): the buyer pays by card there, then the owner manually mints a "30 Day Pass" admin redeem code (existing admin generator) and emails it (≤24h). **Same $4.99 / 30d price on both platforms** — only fulfillment differs (card code by email; crypto grants instantly). The card pass is NOT a fixed crypto catalog item (absent from `CRYPTO_PASS_PLANS`). Passes unlock full history, stats windows, spectating, Find Players posting, @mention, leaderboard windows, and full export; Lifetime adds custom screen names. (Legacy Stripe card checkout + recurring subscriptions exist behind an env flag, currently off; existing subscriptions can still cancel.)
