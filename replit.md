...
- Optional env (restart the api-server workflow after changing any):
  ...
  - `BREAKBPM_BANNED_WORDS` — comma-separated blocklist for user-supplied free text. Whole-word, case-insensitive matching (`wordFilter.ts`), so banning `ass` blocks a standalone "ass" but not "passes"; add explicit variants (e.g. `shitty`) to catch them. Empty/unset → no filtering. **Two behaviours**: HUD ad copy is *cleaned* (each blocked word swapped for a random friendly emoji, never rejected — `cleanBannedWords`); custom screen names are *rejected* with "choose another name" (emoji can't live in the public `/watch/{name}` URL handle — `findBannedWord`).
