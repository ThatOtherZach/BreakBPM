...
- Optional env (restart the api-server workflow after changing any):
  ...
  - `BREAKBPM_STORE_URL` — off-platform card store (Squarespace) URL for the **30 Day Pass** ($4.99 card buy); read per-request via `GET /config`. Empty when unset → the PassesScreen card-store callout is hidden. Buyer pays by card on the store; owner manually mints a "30 Day Pass" admin redeem code and emails it.
