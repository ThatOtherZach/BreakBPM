...
- Optional env (restart the api-server workflow after changing any):
  ...
  - `BREAKBPM_BANNED_WORDS` — comma-separated blocklist for user-supplied free text. Case-insensitive **letter-boundary** matching (`wordFilter.ts`): a banned word matches unless a letter sits directly on either side, so banning `ass` catches a standalone "ass" AND digit/symbol-wrapped uses like "45ass56" / "ass!!", but never "passes"/"class"/"grass" (letters adjacent). Inflections like "shitty" still need explicit entries. Empty/unset → no filtering. **Three surfaces**: HUD ad copy is *cleaned* server-side (each blocked word swapped for a random friendly emoji, never rejected — `cleanBannedWords`); in-game player names are *cleaned* client-side at the SetupScreen input (mirror in `breakbpm/src/lib/wordFilter.ts`, list delivered via `GET /config`); custom screen names are *rejected* with "choose another name" (emoji can't live in the public `/watch/{name}` URL handle — `findBannedWord`).
