...
- Optional env (restart the api-server workflow after changing any):
  ...
  - `BREAKBPM_BANNED_WORDS` — comma-separated blocklist for user-supplied free text (HUD ad copy + custom screen names). Whole-word, case-insensitive matching (`wordFilter.ts`), so banning `ass` blocks a standalone "ass" but not "passes"; add explicit variants (e.g. `shitty`) to catch them. Empty/unset → no filtering.
