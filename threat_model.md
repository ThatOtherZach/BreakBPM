...
- **Public surfaces:** `/games/resolve`, `/games/join`, `/games/state`, `/games/watch-resolve`, `/games/profile`, `/stats` (limited mode), `/leaderboard`, `/leaderboard/hall/:venueId` (per-hall board, slug- or legacy-id-resolvable; its 30-day window is sign-in-free as a crawlable SEO surface, while 90d/all-time are pass-gated), `/leaderboard/city/:locality` (per-city roll-up board — a discovery surface, but **every window requires a signed-in caller**, so it is not anonymous-public), `/passes/plans`, `/config`, Find Players listing.
